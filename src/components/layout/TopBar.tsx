...
import { NotificationCenter } from '@/components/pm/NotificationCenter';

export const TopBar = () => {
  const pathname = usePathname();
  const router = useRouter();
  const product = getAppProduct(pathname);
  const { emailNotifications, notifications, signOut } = useAppState();
    ...
    <>
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b bg-card/40 px-4 backdrop-blur-md md:px-6">
        <div className="flex min-w-0 flex-1 items-center gap-3 md:max-w-3xl md:gap-4">
          ...
          <button
            type="button"
            onClick={() => product === 'projects' ? setCmdOpen(true) : openQuickNav()}
        ...
        <div className="ml-3 flex items-center gap-2 md:gap-4">
          <Button
            size="sm"
            className={`${product === 'business' ? 'gradient-amber text-black shadow-amber-500/20' : 'gradient-pink-blue text-white shadow-primary/20'} hidden h-10 rounded-xl px-4 font-medium shadow-lg sm:inline-flex`}
          >
          ...
          <Button
            variant="ghost"
            size="icon"
            className="relative h-10 w-10 rounded-xl text-muted-foreground hover:text-foreground touch-manipulation"
            onClick={() => setNotifOpen(true)}
          >
            <Bell className="h-5 w-5" />
            {(unreadNotifCount > 0 || emailNotifications.length > 0) && (
              <span className="absolute -right-1 -top-1 min-w-5 rounded-full bg-primary px-1 text-center text-[10px] leading-5 text-primary-foreground">
                {unreadNotifCount || emailNotifications.length}
              </span>
            )}
          </Button>
