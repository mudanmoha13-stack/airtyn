...
export function RestaurantControlTower({ mode = 'control' }: { mode?: 'control' | 'expo' }) {
  ...
  const loadBranchData = useCallback(async (branchId: string) => {
    ...
    if (mode === 'control') {
      const [tableRes, reservationRes, sessionRes, deliveryRes, analyticsRes, ticketRes] = result as [{ tables: TableRecord[] }, { reservations: Reservation[] }, { sessions: PosSession[] }, { deliveries: DeliveryRecord[] }, { analytics: BranchAnalytics }, { tickets: KitchenTicket[] }];
    ...
    <Shell>
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-300">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <Badge className="mb-2 w-fit rounded-full border-primary/20 bg-primary/10 px-3 py-1 text-primary hover:bg-primary/10">Restaurant Phase 3</Badge>
            <h1 className="text-3xl font-bold">{mode === 'expo' ? 'Restaurant Expo Board' : 'Restaurant Control Tower'}</h1>
            <p className="text-sm text-muted-foreground">{mode === 'expo' ? 'Dedicated expo screen for kitchen coordination, SLA monitoring, and ready-to-serve control.' : 'Run branch setup, recipe-driven dine-in settlement, kitchen sequencing, delivery operations, waiter handoff, and branch performance monitoring from one workflow.'}</p>
          </div>
