...
export async function POST(request: NextRequest) {
  try {
    ...
    if (apiKey) {
      ...
      const normalizedBusinessType = payload.businessType?.toLowerCase();
      const templateNote = normalizedBusinessType === 'restaurant' || normalizedBusinessType === 'coffee'
        ? `<p><strong>Template activated:</strong> ${normalizedBusinessType === 'coffee' ? 'Coffee shop' : 'Restaurant'} Odoo-style operations (POS, tables, kitchen, inventory, procurement, finance, HR, CRM, analytics).</p>`
        : '';
